...
function updatePostSeenUserRecent(&$posts, $universeID) {
    ...
    // first check post_seen_user_recent_count table that how many rows
    // post_seen_user_recent table has
    $sql = "SELECT post_seen_count FROM post_seen_user_recent_count WHERE user_id = ? AND universe_id = ?";


    if(!($stmt = $conn->prepare($sql))) {
        $conn->close();
        return "can't prepare statement for retrieving post_seen_count from post_seen_user_recent_count table";
    }
    ...
    if(!$stmt->bind_result($postSeenCount)) {
        $stmt->close();
        $conn->close();
        return "couldn't bind results i.e. postSeenCount with statement in post_seen_user_recent_count table";
    }
    // now fetch data
    ...
    // now, its time to insert/update post_seen_user_recent_count table
    if(!$fetchStatus) {
        // i.e. user has no previous record of seeing posts
        // so, we need to insert data into post_seen_user_recent_count_table

        // assign proper value of $postSeenCount
        $postSeenCount = $countNewPosts;
        
        $sql = "INSERT INTO post_seen_user_recent_count (post_seen_count, update_datetime, user_id, universe_id) VALUES (?, ?, ?, ?)";
        ...
        if($postSeenCount >= 4096 && $prevPostSeenCount > 2048) { // if by adding
            ...
            $sql = "DELETE FROM post_seen_user_recent
            ...
            if(!($stmt = $conn->prepare($sql))) {
                $conn->close();
                return "can't prepare statement for deleting extra(cause, post_seen_count >= 4096) seen posts from post_seen_user_recent table";
            }

            // binding parameters
            $stmt->bind_param("ssss", $currentUserID, $universeID, $currentUserID, $universeID);
            
            if(!$stmt->execute()) {
                $stmt->close();
                $conn->close();
                return "couldn't execute statement for deteting extra (cause, post_seen_count >= 4096) seen posts from post_seen_user_recent table";
            }
        ...
        $sql = "UPDATE post_seen_user_recent_count SET post_seen_count = ?, update_datetime = ? WHERE user_id = ? AND universe_id = ?";
    }

    // prepare sql
    if(!($stmt = $conn->prepare($sql))) {
        $conn->close();
        return "can't prepare statement for inserting/updating data into post_seen_user_recent_count table";
    }

    // binding params
    $stmt->bind_param("isss", $postSeenCount, $todayDatetime, $currentUserID, $universeID);

    if(!$stmt->execute()) {
        $stmt->close();
        $conn->close();
        return "couldn't execute statement for inserting/updating data into post_seen_user_recent_count table";
    }
    ...
    // now, we've to remember that we've to insert new data inside post_seen_user_recent table
    $sql = "INSERT INTO post_seen_user_recent (post_id, user_id, universe_id, send_datetime) VALUES (?, ?, ?, ?)";
    
    if(!($stmt = $conn->prepare($sql))) {
        $conn->close();
        return "can't prepare statement for inserting data into post_seen_user_recent table";
    }
    ...
    $stmt->bind_param("ssss", $postID, $currentUserID, $universeID, $todayDatetime);
    ...
    foreach($posts as &$post) {
        ...
        if(!$stmt->execute()) {
            $stmt->close();
            $conn->close();
            return "couldn't execute statement for inserting values in post_seen_user_recent table";
        }
    }
